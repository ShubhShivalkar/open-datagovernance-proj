...
export default function Step2Reference({ form, setForm, onBack, onNext }) {
  ...
  function validate() {
    if (!form.identifier_table) return "Please select the master reference table.";
    if (!form.identifier_column) return "Please select the identifier column.";
    return "";
  }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-muted/40 border border-border rounded-lg px-4 py-3 text-sm text-muted-foreground">
        Select the table that acts as the root identity record — the engine will traverse all FK
        relationships from this table to find and delete the principal's data everywhere.
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-foreground mb-1.5">Master Reference Table</label>
          {isLoading ? (
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-1.5">
          Reason for Erasure
          <span className="text-muted-foreground font-normal ml-1.5 text-xs">— optional, stored in the audit log</span>
        </label>
        <textarea
          className={`${inputCls} min-h-[80px] resize-y`}
          placeholder="e.g. User exercised right to erasure under DPDPA Section 12…"
          value={form.reason}
      ...
      <div className="flex justify-between pt-2">
        ...
        <button type="button" onClick={handleNext} className="btn btn-primary flex items-center gap-2">
          Compile Plan <ChevronRight size={15} />
        </button>
      </div>
    </div>
  );
